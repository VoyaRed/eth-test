// backtester_2.js - True Historical Forward/Backtester
...
// Core Engine Simulator (Trigger & Filter Architecture)
// 100% Untouched from your original logic
...
async function runBacktest() {
    ...
    // Swapped exchange to Crypto.com, ticker to ETH/USD
    const exchange = new ccxt.cryptocom({
        fetchImplementation: impersonator.fetch.bind(impersonator)
    });
    
    let allCandles = [];
    let since = exchange.milliseconds() - (3000 * 5 * 60 * 1000); // Fetching 3000 candles 
    ...
    try {
        while (allCandles.length < 3000) {
            ...
            await new Promise(r => setTimeout(r, 1000)); // Minor rate limit safety
        }
    } catch (e) {
        console.log("Fetch Error (Ensure Crypto.com supports deep pagination for this pair):", e.message);
    }
    ...
    const testPhase = (dataArray, phaseName) => {
        ...
        let position = null; // Tracks open UpDown Option contracts
        ...
        for (let i = 50; i < dataArray.length - 1; i++) {
            ...
            if (position) {
                ...
                if (position.type === 'UP') {
                    // If it wicks to both boundaries in the same candle, conservatively assume SL hit first.
                    if (low <= position.sl) {
                        losses++;
                        tradeClosed = true;
                ...
                if (tradeClosed) {
                    position = null; // Clear position so bot can look for the next setup
                }
                continue; // Do not scan for new predictions while a contract is active
            }
